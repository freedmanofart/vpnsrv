...
Текущая единственная нода `test-node-01` имеет `region=test`. Новый бот намеренно показывает только активные ноды США, Нидерландов и Германии, поэтому до создания нод с `region=us`, `nl` или `de` список стран будет пуст. Тестовую ноду автоматически не переименовывали, чтобы не выдавать её за сервер другой страны.
