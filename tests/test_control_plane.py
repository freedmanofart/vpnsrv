...
class ControlPlaneTests(IsolatedAsyncioTestCase):
    ...
    async def test_root_serves_landing_and_admin_stays_protected(self) -> None:
        ...
        phone_asset = await self.client.get("/static/landing-phone.png")
        self.assertEqual(200, phone_asset.status_code)
        self.assertEqual("image/png", phone_asset.headers["content-type"])
        logo_asset = await self.client.get("/static/freedom-vpn-mark.webp")
        self.assertEqual(200, logo_asset.status_code)
        self.assertEqual("image/webp", logo_asset.headers["content-type"])
