...
:root{{--color-page:#edf1f7;--color-surface:#fff;--color-surface-blue:#eef5ff;--color-text:#142039;--color-text-secondary:#64708a;--color-primary:#245bff;--color-primary-hover:#164ae8;--color-primary-active:#103dcc;--color-primary-soft:#e9f0ff;--color-border:#d8e0ee;--color-border-strong:#b9c7dc;--color-success:#19b77a;--color-success-soft:#dff7ed;--color-phone-bg:#031831;--color-phone-surface:#082849;--color-focus:#77a0ff;--radius-button:12px;--radius-card:16px;--radius-panel:16px;--radius-shell:20px;--shadow-surface:0 18px 60px rgb(20 32 57 / .08);--shadow-card-hover:0 8px 20px rgb(36 91 255 / .12);--shadow-phone:0 28px 40px rgb(20 32 57 / .18)}}
body:has(.landing-v2){{background:var(--color-page);color:var(--color-text)}}
...
.login-shell .login-card{{max-width:480px;padding:24px;border:0;border-radius:var(--radius-shell);box-shadow:0 8px 24px #17345f0b}}
...
.login-shell .login-card input{{min-height:44px;border:1.5px solid var(--color-primary);border-radius:11px;font-size:14px}}
.login-shell .login-tabs{{margin-top:16px;padding:3px;border:1.5px solid var(--color-primary);border-radius:11px;background:#fff}}
...
.login-shell .gradient{{margin-top:10px;background:var(--color-primary);border-color:var(--color-primary)}}
.login-shell .actions{{justify-content:center}}
.login-shell .actions .button{{width:100%}}
.login-shell .password-heading{{margin-top:18px}}
.login-shell .password-heading button{{font-size:12px}}
@media(max-width:767px){{.login-shell{{width:100%;min-height:100dvh;margin:0;border-radius:0;box-shadow:none}}.login-shell .login-page{{min-height:calc(100dvh - 64px);padding:24px 12px 40px}}.login-shell .login-card{{max-width:460px;padding:20px;border-radius:16px}}.login-shell .login-card h1{{font-size:26px}}}}
.landing-v2{{width:calc(100% - 64px);max-width:1200px;min-height:calc(100dvh - 48px);margin:24px auto;background:var(--color-surface);border-radius:var(--radius-shell);box-shadow:var(--shadow-surface);overflow:hidden}}
.l-sr-only{{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}}
.landing-v2 svg{{display:block}}
.landing-v2 a:focus-visible,.landing-v2 button:focus-visible{{outline:3px solid var(--color-focus);outline-offset:3px}}
.l-header{{height:72px;padding:0 32px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--color-border)}}
.l-brand{{display:flex;align-items:center;gap:12px;min-width:0;color:var(--color-text);font-size:20px;font-weight:800;letter-spacing:-.04em}}
.l-brand img{{width:32px;height:32px;object-fit:contain;flex:none}}
.l-brand i{{font-style:normal;color:var(--color-primary)}}
.l-cabinet{{width:136px;height:44px;display:inline-flex;align-items:center;justify-content:center;border:1.5px solid var(--color-primary);border-radius:var(--radius-button);color:var(--color-primary);font-size:15px;font-weight:650;transition:background 160ms ease,color 160ms ease,transform 160ms ease}}
.l-cabinet:hover{{background:var(--color-primary);color:#fff;transform:translateY(-1px)}}
.l-cabinet:active{{background:var(--color-primary-active);color:#fff;transform:none}}
.l-hero{{min-height:470px;padding:48px 40px 32px;display:grid;grid-template-columns:minmax(0,1fr) 480px;gap:56px;align-items:center;background:linear-gradient(135deg,#fff 0%,#f3f7ff 58%,#fff 100%)}}
.l-hero-copy{{max-width:520px}}
.l-eyebrow{{margin:0 0 20px;color:#365995;font-size:14px;line-height:20px;font-weight:600;letter-spacing:.06em;text-transform:uppercase}}
.l-hero h1{{max-width:520px;margin:0 0 22px;color:var(--color-text);font-size:56px;line-height:60px;font-weight:720;letter-spacing:-.035em}}
.l-hero h1 span{{color:var(--color-primary)}}
.l-hero-text{{max-width:520px;margin:0 0 26px;color:var(--color-text);font-size:20px;line-height:30px;font-weight:400}}
.l-cta{{width:308px;max-width:100%;height:56px;padding:0 24px;display:inline-flex;align-items:center;justify-content:center;gap:14px;border:0;border-radius:var(--radius-button);background:var(--color-primary);color:#fff;font:650 17px/24px inherit;cursor:pointer;transition:background 160ms ease,transform 160ms ease}}
.l-cta svg{{width:20px;height:20px;flex:none}}
.l-cta:hover{{background:var(--color-primary-hover);transform:translateY(-1px)}}
.l-cta:active{{background:var(--color-primary-active);transform:none}}
.l-platform-line{{width:308px;max-width:100%;margin:14px 0 0;text-align:center;color:#365995;font-size:14px;line-height:20px}}
.l-phone-stage{{min-width:0;display:flex;align-items:center;justify-content:center;position:relative}}
.l-phone-glow{{position:absolute;width:410px;height:210px;border-radius:50%;background:radial-gradient(ellipse,rgb(36 91 255 / .13) 0%,rgb(36 91 255 / .055) 48%,transparent 76%);filter:blur(48px);bottom:4px}}
.l-phone{{width:286px;height:560px;position:relative;overflow:hidden;filter:drop-shadow(0 28px 30px rgb(20 58 130 / .18)) drop-shadow(0 8px 14px rgb(20 32 57 / .12));transform:perspective(1000px) rotate(4deg) rotateY(-5deg);animation:l-phone-in 450ms ease both}}
.l-phone img{{position:absolute;width:136.4%;max-width:none;height:auto;left:-18%;top:-4.1%}}
@keyframes l-phone-in{{from{{opacity:0;transform:perspective(1000px) rotate(4deg) rotateY(-5deg) translateY(12px)}}to{{opacity:1;transform:perspective(1000px) rotate(4deg) rotateY(-5deg) translateY(0)}}}}
.l-benefits{{margin-top:24px;padding:0 40px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}}
.l-benefit{{height:112px;padding:22px 24px;display:flex;align-items:center;gap:18px;border:1px solid var(--color-border);border-radius:var(--radius-card);background:var(--color-surface)}}
.l-benefit-icon{{width:32px;height:32px;flex:none;color:var(--color-primary)}}
.l-benefit h2{{margin:0;color:var(--color-text);font-size:18px;line-height:24px;font-weight:700;letter-spacing:0}}
.l-benefit p{{margin:1px 0 0;color:var(--color-text-secondary);font-size:15px;line-height:22px}}
.l-downloads{{min-height:190px;margin:24px 40px 0;padding:26px 24px 28px;border:1.5px solid var(--color-primary);border-radius:var(--radius-panel);background:linear-gradient(135deg,#f4f8ff 0%,#e5efff 100%)}}
.l-downloads h2{{margin:0;text-align:center;color:var(--color-text);font-size:30px;line-height:38px;font-weight:700;letter-spacing:-.025em}}
.l-downloads>p{{margin:6px 0 20px;text-align:center;color:var(--color-text-secondary);font-size:15px;line-height:22px}}
.l-download-grid{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}}
.l-download-card{{height:72px;padding:12px 16px;display:flex;align-items:center;gap:12px;border:1px solid transparent;border-radius:var(--radius-button);background:#fff;transition:border-color 160ms ease,box-shadow 160ms ease,transform 160ms ease}}
.l-download-card:hover{{border-color:var(--color-primary);box-shadow:var(--shadow-card-hover);transform:translateY(-1px)}}
.l-platform-icon{{width:28px;height:28px;flex:none;color:var(--color-text)}}
.l-platform-icon.android{{color:var(--color-success)}}
.l-download-copy{{min-width:0;margin-right:auto}}
.l-download-copy strong,.l-download-copy span{{display:block}}
.l-download-copy strong{{color:var(--color-text);font-size:15px;line-height:20px;font-weight:650}}
.l-download-copy span{{color:var(--color-text-secondary);font-size:13px;line-height:18px}}
.l-download-arrow{{width:20px;height:20px;flex:none;color:var(--color-primary)}}
.l-footer{{min-height:64px;padding:0 40px;display:flex;align-items:center;justify-content:center;color:var(--color-text-secondary);font-size:13px;line-height:18px}}
.l-footer-links{{display:flex;align-items:center;justify-content:center;gap:24px;flex-wrap:wrap;text-align:center}}
.l-footer a:hover{{color:var(--color-primary)}}
@media(max-width:1023px){{.landing-v2{{width:calc(100% - 48px)}}.l-hero{{grid-template-columns:minmax(0,1fr) 360px;gap:28px;padding-inline:32px}}.l-hero h1{{font-size:48px;line-height:52px}}.l-phone{{width:245px;height:480px}}.l-benefits{{padding-inline:32px}}.l-benefit{{padding-inline:16px}}.l-downloads{{margin-inline:32px}}}}
@media(max-width:767px){{.landing-v2{{width:100%;min-height:100dvh;margin:0;border-radius:0;box-shadow:none}}.l-header{{height:64px;padding:0 16px}}.l-brand{{gap:8px;font-size:17px}}.l-brand img{{width:28px;height:28px}}.l-cabinet{{width:92px;height:38px;border-radius:10px;font-size:14px}}.l-hero{{min-height:0;padding:24px 16px 20px;display:grid;grid-template-columns:1fr;gap:18px;align-items:start}}.l-hero-copy{{max-width:none}}.l-eyebrow{{margin-bottom:10px;font-size:11px;line-height:16px;letter-spacing:.045em}}.l-hero h1{{margin-bottom:12px;font-size:36px;line-height:39px;font-weight:700;letter-spacing:-.03em}}.l-hero-text{{margin-bottom:18px;font-size:16px;line-height:23px}}.l-cta{{width:100%;height:52px;font-size:16px;line-height:22px}}.l-platform-line{{margin-top:10px;text-align:center;font-size:12px;line-height:18px}}.l-phone-stage{{height:348px}}.l-phone-glow{{width:250px;height:118px;bottom:10px;filter:blur(28px)}}.l-phone{{width:174px;height:348px;transform:rotate(1deg)}}.l-phone img{{width:136.4%;left:-18%;top:-4.1%}}.l-benefits{{padding:0 16px;grid-template-columns:1fr;gap:8px}}.l-benefit{{min-height:68px;height:auto;padding:12px 14px;gap:14px;border-radius:12px}}.l-benefit-icon{{width:30px;height:30px}}.l-benefit h2{{font-size:15px;line-height:20px}}.l-benefit p{{font-size:13px;line-height:18px}}.l-downloads{{min-height:0;margin:16px;padding:18px 10px 12px;border-radius:16px}}.l-downloads h2{{font-size:22px;line-height:27px}}.l-downloads>p{{margin:4px 0 14px;font-size:13px;line-height:18px}}.l-download-grid{{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}}.l-download-card{{height:64px;padding:9px 10px;gap:8px}}.l-platform-icon{{width:24px;height:24px}}.l-download-arrow{{width:18px;height:18px}}.l-download-copy strong{{font-size:13px;line-height:17px}}.l-download-copy span{{font-size:11px;line-height:15px}}.l-footer{{min-height:72px;padding:20px 16px;align-items:flex-start;flex-direction:column;gap:12px;font-size:12px;line-height:18px}}.l-footer-links{{justify-content:flex-start;gap:8px 12px}}}}
...
async def landing(db: AsyncSession = Depends(get_db)):
    ...
    body = f"""<div class=\"landing-v2\">
<header class=\"l-header\"><a class=\"l-brand\" href=\"/\" aria-label=\"Freedom VPN — главная\"><img src=\"/static/freedom-vpn-mark.webp\" srcset=\"/static/freedom-vpn-mark.webp 1x, /static/freedom-vpn-mark@2x.webp 2x\" width=\"32\" height=\"32\" alt=\"\"><span>Freedom <i>VPN</i></span></a><a class=\"l-cabinet\" href=\"/cabinet\">Кабинет</a></header>
<main>
<section class=\"l-hero\" aria-labelledby=\"landing-title\"><div class=\"l-hero-copy\"><p class=\"l-eyebrow\">Свободный интернет без ограничений</p><h1 id=\"landing-title\">Быстрый и приватный <span>интернет</span></h1><p class=\"l-hero-text\">Freedom VPN для ваших устройств.<br>Подключайтесь и управляйте подпиской в личном кабинете.</p><button class=\"l-cta\" type=\"button\" data-choose-plan>Выбрать подписку<svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.75\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M5 12h14M13 6l6 6-6 6\"></path></svg></button><p class=\"l-platform-line\">macOS · iOS · Windows · Android</p></div><div class=\"l-phone-stage\"><span class=\"l-phone-glow\" aria-hidden=\"true\"></span><div class=\"l-phone\"><img src=\"/static/landing-phone.png\" srcset=\"/static/landing-phone.png 1x, /static/landing-phone@2x.png 2x\" width=\"512\" height=\"768\" alt=\"Приложение Freedom VPN на смартфоне\"></div></div></section>
<section class=\"l-benefits\" aria-labelledby=\"benefits-title\"><h2 class=\"l-sr-only\" id=\"benefits-title\">Преимущества Freedom VPN</h2>
<article class=\"l-benefit\"><svg class=\"l-benefit-icon\" viewBox=\"0 0 32 32\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><rect x=\"2\" y=\"11\" width=\"19\" height=\"13\" rx=\"2\"></rect><path d=\"M7 28h9M11.5 24v4\"></path><rect x=\"23\" y=\"5\" width=\"7\" height=\"19\" rx=\"2\"></rect></svg><div><h2>Один аккаунт</h2><p>Для ваших устройств</p></div></article>
<article class=\"l-benefit\"><svg class=\"l-benefit-icon\" viewBox=\"0 0 32 32\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><circle cx=\"16\" cy=\"16\" r=\"5\"></circle><path d=\"M16 3v4M16 25v4M3 16h4M25 16h4M6.8 6.8l2.8 2.8M22.4 22.4l2.8 2.8M25.2 6.8l-2.8 2.8M9.6 22.4l-2.8 2.8\"></path></svg><div><h2>Удобное управление</h2><p>Подписка и ключ в кабинете</p></div></article>
<article class=\"l-benefit\"><svg class=\"l-benefit-icon\" viewBox=\"0 0 32 32\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M8 3h11l6 6v20H8z\"></path><path d=\"M19 3v7h6M12 16h9M12 21h9\"></path></svg><div><h2>Простое подключение</h2><p>Инструкции для устройств</p></div></article>
</section>
<section class=\"l-downloads\" aria-labelledby=\"downloads-title\"><h2 id=\"downloads-title\">Приложение для всех ваших устройств</h2><p>Установите Freedom VPN на компьютер и смартфон.</p><div class=\"l-download-grid\">
<a class=\"l-download-card\" href=\"https://github.com/INCY-DEV/incy-platforms/releases/latest/download/incy-macos-arm64.dmg\"><svg class=\"l-platform-icon\" viewBox=\"0 0 24 24\" fill=\"currentColor\" aria-hidden=\"true\"><path d=\"M16.7 12.1c0-2.6 2.1-3.8 2.2-3.9-1.2-1.8-3.2-2-3.9-2-1.7-.2-3.2 1-4 1s-2.1-1-3.5-1c-1.8 0-3.5 1.1-4.4 2.7-1.9 3.3-.5 8.1 1.3 10.7.9 1.3 2 2.8 3.4 2.7 1.4-.1 1.9-.9 3.6-.9s2.2.9 3.6.9c1.5 0 2.5-1.3 3.4-2.6 1-1.5 1.5-3 1.5-3.1-.1 0-3.2-1.2-3.2-4.5ZM14 4.5c.8-1 1.3-2.3 1.2-3.5-1.2.1-2.6.8-3.4 1.7-.7.8-1.3 2.1-1.2 3.4 1.3.1 2.6-.7 3.4-1.6Z\"></path></svg><span class=\"l-download-copy\"><strong>macOS</strong><span>Скачать</span></span><svg class=\"l-download-arrow\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.75\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M12 3v13M7 11l5 5 5-5M5 21h14\"></path></svg></a>
<a class=\"l-download-card\" data-native-store href=\"https://apps.apple.com/ca/app/incy/id6756943388\" target=\"_blank\" rel=\"noopener noreferrer\"><svg class=\"l-platform-icon\" viewBox=\"0 0 24 24\" fill=\"currentColor\" aria-hidden=\"true\"><path d=\"M16.7 12.1c0-2.6 2.1-3.8 2.2-3.9-1.2-1.8-3.2-2-3.9-2-1.7-.2-3.2 1-4 1s-2.1-1-3.5-1c-1.8 0-3.5 1.1-4.4 2.7-1.9 3.3-.5 8.1 1.3 10.7.9 1.3 2 2.8 3.4 2.7 1.4-.1 1.9-.9 3.6-.9s2.2.9 3.6.9c1.5 0 2.5-1.3 3.4-2.6 1-1.5 1.5-3 1.5-3.1-.1 0-3.2-1.2-3.2-4.5ZM14 4.5c.8-1 1.3-2.3 1.2-3.5-1.2.1-2.6.8-3.4 1.7-.7.8-1.3 2.1-1.2 3.4 1.3.1 2.6-.7 3.4-1.6Z\"></path></svg><span class=\"l-download-copy\"><strong>iOS</strong><span>Скачать</span></span><svg class=\"l-download-arrow\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.75\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M12 3v13M7 11l5 5 5-5M5 21h14\"></path></svg></a>
<a class=\"l-download-card\" href=\"https://github.com/INCY-DEV/incy-platforms/releases/latest/download/incy-windows-setup.exe\"><svg class=\"l-platform-icon\" viewBox=\"0 0 24 24\" fill=\"currentColor\" aria-hidden=\"true\"><path d=\"M2 4.2 10.5 3v8.2H2zm9.5-1.4L22 1.3v9.9H11.5zM2 12.2h8.5V21L2 19.8zm9.5 0H22v10.5l-10.5-1.5z\"></path></svg><span class=\"l-download-copy\"><strong>Windows</strong><span>Скачать</span></span><svg class=\"l-download-arrow\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.75\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M12 3v13M7 11l5 5 5-5M5 21h14\"></path></svg></a>
<a class=\"l-download-card\" data-native-store href=\"https://play.google.com/store/apps/details?id=llc.itdev.incy\" target=\"_blank\" rel=\"noopener noreferrer\"><svg class=\"l-platform-icon android\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.75\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M6 9h12v9H6zM8 9V7a4 4 0 0 1 8 0v2M8.5 4 7 2M15.5 4 17 2M4 10v6M20 10v6M9 18v4M15 18v4\"></path><circle cx=\"9.5\" cy=\"7\" r=\".5\" fill=\"currentColor\" stroke=\"none\"></circle><circle cx=\"14.5\" cy=\"7\" r=\".5\" fill=\"currentColor\" stroke=\"none\"></circle></svg><span class=\"l-download-copy\"><strong>Android</strong><span>Скачать</span></span><svg class=\"l-download-arrow\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.75\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M12 3v13M7 11l5 5 5-5M5 21h14\"></path></svg></a>
</div></section>
</main>
<footer class=\"l-footer\"><nav class=\"l-footer-links\" aria-label=\"Документы и контакты\"><a href=\"/static/privacy.html\">Конфиденциальность</a><a href=\"/static/terms.html\">Условия использования</a><a href=\"https://t.me/Freedom_VPN_Support\" target=\"_blank\" rel=\"noopener noreferrer\">Контакты</a></nav></footer>
</div>
...
async def password_setup_page(
    ...
    body = '''<div class="login-shell"><header class="l-header"><a class="l-brand" href="/" aria-label="Freedom VPN — главная"><img src="/static/freedom-vpn-mark.webp" srcset="/static/freedom-vpn-mark.webp 1x, /static/freedom-vpn-mark@2x.webp 2x" width="32" height="32" alt=""><span>Freedom <i>VPN</i></span></a><a class="l-cabinet" href="/cabinet">Кабинет</a></header><main class="login-page"><section class="login-card"><h1>Создайте пароль</h1><p class="muted">После этого вы сможете выбирать вход по письму или паролю.</p><label for="new-password">Пароль</label><input id="new-password" type="password" minlength="8" maxlength="128" autocomplete="new-password" placeholder="Не менее 8 символов"><button class="button gradient" type="button" onclick="savePassword()">Сохранить и открыть кабинет</button><p id="password-result"></p><p class="login-footer"><a href="/cabinet">Настроить позже</a></p></section></main></div><script>async function savePassword(){const out=document.getElementById('password-result');out.textContent='Сохраняем…';const r=await fetch('/web/password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:document.getElementById('new-password').value})});const d=await r.json();if(r.ok){location.href='/cabinet';return}out.className='error';out.textContent=d.detail||'Не удалось сохранить пароль'}</script>'''
