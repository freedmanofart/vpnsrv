...
def _cabinet_payment_rows(payments: list[Payment]) -> str:
    ...
    for payment in payments:
        ...
        rows.append(
            ...
            f'<b>#{payment.id}</b>'
            f'<span>{payment.amount:g} {html.escape(payment.currency)}</span>'
...
async def cabinet(
    ...
    if vpn_uri:
        key_block = (
            ...
            '<button class="button copy-button" type="button" onclick="copyKey(this)"><span aria-hidden="true">▣</span> Копировать ключ</button>'
            '</div>'
        )
    body = f"""
<div class="cabinet-page">
<header class="cabinet-header"><a class="brand" href="/"><img src="/static/freedom-vpn-logo-web.webp" alt="">Freedom <i>VPN</i></a><form method="post" action="/cabinet/logout"><button class="cabinet-logout" type="submit">Выйти</button></form></header>
...
<section class="cabinet-card renew-card" id="payment"><div class="renew-heading"><h2>Продлить подписку</h2><p>Выберите способ оплаты</p></div><input type="hidden" id="order-plan" value="{initial_plan_id}"><div class="payment-picker"><select id="order-method" aria-label="Выберите способ оплаты">{method_options}</select><span class="payment-method-icon" id="payment-method-icon" aria-hidden="true">▦</span><span class="payment-method-copy"><strong id="payment-method-name">Способ оплаты</strong><small id="payment-method-hint">Нажмите, чтобы выбрать</small></span><span class="payment-check" aria-hidden="true">✓</span><span class="payment-chevron" aria-hidden="true">›</span></div><div class="renew-order"><div><small>Тариф</small><strong id="renew-plan-name">—</strong></div><div class="renew-amount"><small>К оплате</small><strong id="renew-amount">—</strong></div></div><button class="pay-button" id="pay-button" type="button" onclick="openPayment()">Оплатить</button><a class="change-plan" href="/cabinet/tariffs">Изменить тариф</a></section>
<section class="cabinet-card connection-card"><h2>Подключение</h2>{key_block}<p class="server-line"><span aria-hidden="true">🌐</span> Сервер: {html.escape(node.region or node.name) if node else 'назначится после оплаты'}</p></section>
<section class="cabinet-card apps-card"><h2>Скачать приложение</h2><div class="app-grid"><a class="app-button" href="https://github.com/amnezia-vpn/amnezia-client/releases/download/4.8.10.0/AmneziaVPN_4.8.10.0_windows_x64.exe"><span>⊞</span>Windows</a><a class="app-button" href="https://github.com/amnezia-vpn/amnezia-client/releases/download/4.8.10.0/AmneziaVPN_4.8.10.0_macos.zip"><span>●</span>macOS</a><a class="app-button" data-native-store href="https://play.google.com/store/apps/details?id=llc.itdev.incy" target="_blank" rel="noopener noreferrer"><span>♟</span>Android</a><a class="app-button" data-native-store href="https://apps.apple.com/ca/app/incy/id6756943388" target="_blank" rel="noopener noreferrer"><span>●</span>iOS</a></div></section>
