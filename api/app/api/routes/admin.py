...
<section id="provider_codes" class="hidden"><h2>Коды подключения провайдера</h2><p class="muted">Укажите только email или только Telegram ID. Новый выпуск отменяет прежние неиспользованные коды пользователя. Полный код показывается один раз и отправляется пользователю по включённым каналам.</p><form id="providerCodeForm" onsubmit="createProviderCode(event)"><label>Email пользователя<input name="email" type="email" placeholder="user@example.com"></label><label>Telegram ID<input name="telegram_id" type="number" step="1" placeholder="123456789"></label><label>Срок, минут<input name="ttl_minutes" type="number" min="1" max="60" value="10" required></label><button>Выпустить код</button></form><div id="provider-code-result" class="script-output hidden"></div><div class="table"></div></section>
...
function renderProviderCodes(){table('provider_codes',state.provider_codes||[],r=>`<button onclick="refreshProviderCode(${r.id})">Выпустить новый</button>`)}
function showIssuedProviderCode(data){let box=document.getElementById('provider-code-result');box.classList.remove('hidden');box.innerHTML=`<b>Код выпущен. Скопируйте сейчас — повторно он не показывается.</b><br><br>Код: <code id="issued-provider-code">${esc(data.code)}</code> <button type="button" onclick="navigator.clipboard.writeText('${esc(data.code)}')">Копировать</button><br>Имя устройства: <code>${esc(data.device_name)}</code><br>Действует до: ${esc(prettyDate(data.expires_at))}<br>Telegram: ${data.telegram_sent?'отправлен':'не отправлен'} · Email: ${data.email_sent?'отправлен':'не отправлен'}`}
async function createProviderCode(e){e.preventDefault();let f=Object.fromEntries(new FormData(e.target));let email=String(f.email||'').trim(),telegram=String(f.telegram_id||'').trim();if(Boolean(email)===Boolean(telegram)){alert('Укажите либо email, либо Telegram ID');return}let body={email:email||null,telegram_id:telegram?Number(telegram):null,ttl_minutes:Number(f.ttl_minutes)};try{let data=await request('/admin/provider-codes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});showIssuedProviderCode(data);e.target.reset();e.target.ttl_minutes.value=10;await load();show('provider_codes')}catch(err){alert(err.message)}}
async function refreshProviderCode(id){let ttl=prompt('Срок действия нового кода, минут','10');if(ttl===null)return;ttl=Number(ttl);if(!Number.isInteger(ttl)||ttl<1||ttl>60){alert('Введите целое число от 1 до 60');return}if(!confirm('Выпустить новый код? Все прежние активные коды пользователя будут отменены.'))return;try{let data=await request('/admin/provider-codes/'+id+'/refresh',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ttl_minutes:ttl})});showIssuedProviderCode(data);await load();show('provider_codes')}catch(err){alert(err.message)}}
...
async function editPlanPackage(id){let p=state.plan_packages.find(x=>x.id===id);let name=prompt('Название пакета',p.name);if(name===null)return;let description=prompt('Описание',p.description||'');if(description===null)return;let limit=prompt('Подключений; 0 без ограничений',p.max_connections);if(limit===null)return;let traffic=prompt('Трафик, ГБ; 0 без ограничений',p.traffic_limit_gb);if(traffic===null)return;let order=prompt('Порядок',p.sort_order);if(order===null)return;let active=confirm('Пакет активен?');await request('/plans/packages/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,description,max_connections:Number(limit),traffic_limit_gb:Number(traffic),sort_order:Number(order),is_active:active})});await load()}
async function editPlan(id){let p=state.plans.find(x=>x.id===id);let name=prompt('Название тарифа',p.name);if(name===null)return;let price=prompt('Цена',p.price);if(price===null)return;let days=prompt('Длительность, дней',p.duration_days);if(days===null)return;let limit=prompt('Одновременных подключений/IP; 0 без ограничений',p.max_connections);if(limit===null)return;let traffic=prompt('Трафик, ГБ; 0 без ограничений',p.traffic_limit_gb);if(traffic===null)return;let packageId=prompt('ID пакета тарифа; пусто без пакета',p.package_id||'');if(packageId===null)return;let active=confirm('Тариф должен быть активен и публичен?');await request('/plans/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,price:Number(price),duration_days:Number(days),max_connections:Number(limit),traffic_limit_gb:Number(traffic),package_id:packageId?Number(packageId):null,is_active:active,is_public:active})});await load()}
...
const loadBase=load;load=async function(){await loadBase();if(!document.getElementById('provider_codes').classList.contains('hidden'))renderProviderCodes()};
