...
async def overview(db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "payment_methods": [{"id": x.id, "code": x.code, "name": x.name, "url": x.url, "sort_order": x.sort_order, "is_active": x.is_active} for x in sorted(payment_methods, key=lambda item: (item.sort_order, item.id))],
...
<section id="payment_methods" class="hidden"><h2>Способы оплаты</h2><form onsubmit="createPaymentMethod(event)"><input name="code" placeholder="Код: sbp" required><input name="name" placeholder="Название кнопки" required><input name="url" placeholder="https://... (можно оставить пустым)"><input name="sort_order" type="number" value="100" required><button>Добавить способ</button></form><p>Порядок и активность управляют кнопками Telegram. Пустая ссылка показывает способ, но не начинает внешнюю оплату.</p><div class="table"></div></section>
...
let state={};const sections=['plans','nodes','users','subscriptions','clients','payments','payment_methods','devices','debug','audit'];
