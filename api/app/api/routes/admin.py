...
<section id="plan_packages"><h2>Пакеты тарифов</h2><form onsubmit="createPlanPackage(event)"><input name="code" placeholder="Код: lite" required><input name="name" placeholder="Название: Лайт" required><input name="description" placeholder="Описание"><input name="max_connections" type="number" min="0" max="100" value="5" required><input name="traffic_limit_gb" type="number" min="0" value="250" required><input name="sort_order" type="number" value="100" required><button>Создать пакет</button></form><p>Пакет — это сущность для группировки тарифов: Лайт, Стандарт, Ультра. В тарифе выберите пакет, к которому он относится.</p><div class="table"></div></section>
...
function createPlanPackage(e){return sendForm(e,()=>'/plans/packages',f=>({...f,max_connections:Number(f.max_connections),traffic_limit_gb:Number(f.traffic_limit_gb),sort_order:Number(f.sort_order),is_active:true}))}
