...
<section id="nodes" class="hidden"><h2>VPN-ноды</h2><form onsubmit="createNode(event)"><input name="name" placeholder="Имя" required><input name="provider" placeholder="Провайдер" required><select name="region"><option value="us">🇺🇸 США</option><option value="nl">🇳🇱 Нидерланды</option><option value="de">🇩🇪 Германия</option></select><input name="ip_address" placeholder="IP" required><input name="hostname" placeholder="Hostname"><input name="capacity" type="number" value="100"><button>Создать ноду</button></form><details><summary>Добавить VLESS Reality конфигурацию</summary><form onsubmit="createConfig(event)"><input name="node_id" type="number" placeholder="Node ID" required><input name="api_address" placeholder="Локальный Xray API host:port" required><input name="host" placeholder="Публичный host" required><input name="port" type="number" value="443" required><input name="sni" placeholder="SNI" required><input name="fingerprint" placeholder="Fingerprint (chrome/firefox/safari/randomized)" value="chrome" required><input name="pbk" placeholder="Public key" required><input name="sid" placeholder="Short ID" required><input name="inbound_tag" value="vless-reality"><button>Добавить</button></form></details><div class="table"></div></section>
