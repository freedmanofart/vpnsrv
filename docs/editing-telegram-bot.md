...
срок → способ оплаты. Сейчас бот сохраняет совместимость со старой логикой:
если у плана есть код `lite_*`, `standard_*`, `ultra_*`, он попадёт в нужный
пакет; также используется число подключений. В backend уже есть отдельная
сущность пакета тарифа `plan_packages` и связь `plans.package_id`, которую
использует web-кабинет и админка. Способы оплаты загружаются из
`GET /payment-methods`; название, ссылку, порядок и активность оператор меняет
в web admin без правки бота.
