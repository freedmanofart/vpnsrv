...
При запуске бот регистрирует команды через Telegram Bot API и включает
`MenuButtonCommands`. Поэтому рядом с полем ввода отображается системная кнопка
...
Команды задаются в `main()` вызовами `set_my_commands()` и
`set_chat_menu_button()`. После изменения списка обязательно перезапустите бота:
Telegram получает новый список только при выполнении этих вызовов.
