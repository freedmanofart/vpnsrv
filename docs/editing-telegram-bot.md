...
и одноразовую двухколоночную клавиатуру непосредственно под приветствием:

| Левая кнопка | Правая кнопка |
|---|---|
| `🌐 Сайт` | `✉️ Email` |

`🌐 Сайт` открывает `WEB_CABINET_URL`. `✉️ Email` переводит пользователя в
...
Обычное постоянное меню остаётся прежним и не содержит web-кабинет:
