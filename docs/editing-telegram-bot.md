...
Порядок покупки: устройство → страна → пакет → срок → способ оплаты. Telegram
загружает пакеты через `GET /plans/packages`, а сами сроки и цены через
`GET /plans`. Названия пакетов и текст под ними берутся из `plan_packages.name`
и `plan_packages.description`; лимиты подключений/трафика используются только
как fallback, если описание пустое. Менять это нужно в web admin, раздел
`Пакеты тарифов`; бот больше не должен дублировать эти значения в коде.
Если у старого плана нет `package_id`, бот
сохраняет fallback по коду тарифа `lite_*`, `standard_*`, `ultra_*` или числу
подключений, но новые тарифы лучше всегда привязывать к `plan_packages`.
Способы оплаты загружаются из `GET /payment-methods`; название, ссылку, порядок
и активность оператор меняет в web admin без правки бота.
...
Для методов `platega_*` бот создаёт платеж Platega и отправляет пользователю
кнопку «Перейти к оплате». Уведомления админу и клиенту отправляются только
после callback `CONFIRMED`; при создании ссылки оплаты уведомления не
отправляются. Return URL для Telegram ведёт обратно в `https://t.me/vpn142323srv_bot`,
а не в web-кабинет.
...
Под QR-кодом показывается кнопка «Скопировать ключ». После нажатия бот повторно
проверяет по Telegram ID, что указанный VPN-клиент всё ещё принадлежит
пользователю и активен, а затем отправляет полный ключ отдельным блоком `<pre>`.
На iPhone такой блок можно скопировать долгим нажатием. Нативная Telegram-кнопка
`copy_text` здесь не используется: Bot API ограничивает её содержимое 256
символами, а полная VLESS-ссылка обычно длиннее.
...
Сначала API ищет код в таблице `promo_codes`, где он привязан к конкретному
тарифу и имеет отдельные общий и пользовательский лимиты. `NULL` в полях лимита
означает отсутствие ограничения. Если записи в БД нет, для совместимости
проверяется старый список `PROMO_CODES`. Промокод `LIGHT1DAY` хранится в БД,
связан с тарифом `lite_1d` и допускает неограниченные повторные применения.
...
## Копирование VPN-ключа на iPhone

Функция `send_key_message()` отправляет QR-код с кнопкой «Скопировать ключ».
Обработчик `vpn_key_copy_handler()` проверяет владельца и активного клиента,
после чего отправляет отдельное текстовое сообщение с полным VLESS-ключом внутри
блока `<pre>`. Ключ нельзя оставлять только в подписи к фотографии: Telegram для
iOS не всегда позволяет корректно скопировать длинный code entity из caption.

Нативный `CopyTextButton` не используется для VLESS-ссылки, поскольку Telegram
Bot API ограничивает его содержимое 256 символами, а фактический ключ обычно
длиннее. Пользователь нажимает или удерживает отдельный блок с ключом и выбирает
`Скопировать`; QR остаётся альтернативным способом импорта.
