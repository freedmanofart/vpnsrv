...
`GET /plans`. Названия пакетов, описание, лимит подключений и лимит трафика
нужно менять в web admin, раздел `Пакеты тарифов`; бот больше не должен
дублировать эти значения в коде. Если у старого плана нет `package_id`, бот
