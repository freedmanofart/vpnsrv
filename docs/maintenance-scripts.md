...
firewalld с политикой `DROP`, оставляет публичным Xray TCP/443 и разрешает SSH
только из явно заданных административных CIDR. Скрипт нельзя применять через
...
Подключитесь к ноде, оставьте терминал открытым и укажите текущий публичный адрес
оператора (не адрес ноды). Скрипт проверит, что текущая SSH-сессия действительно
пришла из разрешённой сети, и не принимает `0.0.0.0/0` или `::/0`:

```bash
SSH_ALLOW_CIDRS=198.51.100.7/32 \
...
```bash
SSH_ALLOW_CIDRS=198.51.100.7/32 XRAY_TCP_PORTS=443,8443 \
  /root/harden_vpn_node_firewall.sh --apply
```
...
провайдера настраивается отдельно; сначала разрешите в нём административный IP.
