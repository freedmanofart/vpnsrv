...
Данные тарифов берутся из таблицы `plans` через backend. Группировка тарифов
берётся из `plan_packages`: `plans.package_id` указывает, к какому пакету
относится конкретный тариф. На лендинге показываются только активные публичные
месячные тарифы, а в кабинете и на странице `/cabinet/tariffs` тарифы
группируются по пакетам:

- `Лайт`;
- `Стандарт`;
- `Ультра`.

Старая эвристика по коду `lite_* / standard_* / ultra_*` и числу подключений
оставлена только как fallback для старых записей без `package_id`.
...
Важно: админка выполняет только безопасные allow-list проверки внутри
API-контейнера: health, SMTP, online API, backup/verify. Host-only операции
уровня Docker/systemd/SSH не выполняются из контейнера: кнопка возвращает
готовую команду для SSH-хоста. Список node SSL renew строится динамически по
активным `vpn_nodes`.
...
В кабинете больше нет отдельного большого блока «Приобрести или продлить».
Продление встроено в первый блок «Подписка»: слева статус, остаток дней,
трафик и число подключений, справа выбор способа оплаты и резюме заказа.
Кнопка называется «Продлить».

Текущий тариф выбирается автоматически. Если пользователь хочет другой пакет
или срок, кнопка «Сменить тариф» открывает отдельную страницу
`/cabinet/tariffs`. После выбора тарифа пользователь возвращается в кабинет с
этим тарифом в блоке продления.

Platega-платежи создаются только после нажатия «Продлить» в окне
подтверждения. Администратор и клиент получают уведомление не при создании
платежа, а после callback `CONFIRMED`.

Для ручной оплаты API отправляет администратору карточку покупки в Telegram и
на `ADMIN_NOTIFICATION_EMAIL`; после загрузки чека отправляется повторная
карточка с вложенным файлом чека и кнопками ручного подтверждения.
...
- Тарифы должны быть сгруппированы по `plan_packages`; fallback по
  `lite_* / standard_* / ultra_*` нужен только для старых записей без
  `package_id`.
