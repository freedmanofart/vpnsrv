...
Таблицы, связи web-кабинета с платежами/подписками и быстрые проверки БД
описаны в [`database-reference.md`](database-reference.md).
...
Пакет тарифа — отдельная сущность `plan_packages`. Сейчас используются три
пакета:

- `lite` — Лайт, 5 подключений, 250 ГБ;
- `standard` — Стандарт, 15 подключений, 650 ГБ;
- `ultra` — Ультра, 30 подключений, 3 ТБ.

Тарифы из `plans` привязаны к пакету через `plans.package_id`. В админке есть
раздел «Пакеты тарифов», где можно менять название, описание, лимиты и порядок.
У конкретного тарифа пакет выбирается в разделе «Тарифы».

## Покупка и продление без Telegram

В кабинете блоки расположены в таком порядке:

1. Подписка + продление и выбор способа оплаты.
2. Ключ доступа + кнопки скачивания приложений.
3. Последние платежи.
4. Смена пароля.

В Telegram Web App внутренние части крупных блоков раскладываются по две
колонки, когда ширина экрана позволяет.

В блоке «Подписка» текущий тариф выбирается автоматически. Пользователь видит
статус, остаток дней, реальный остаток трафика и число подключений. Если трафик
закончился или срок истёк, статус становится «не активна» и окрашивается в
красный.

Справа в этом же блоке пользователь выбирает:

1. активный способ оплаты из `payment_methods`;
2. текущий тариф по умолчанию, либо другой тариф после перехода по кнопке
   «Сменить тариф».

Кнопка «Сменить тариф» открывает `/cabinet/tariffs`: отдельную страницу с
пакетами Лайт, Стандарт и Ультра. Выбор тарифа возвращает пользователя в
`/cabinet?plan_id=<id>#payment`, и блок продления обновляется выбранным планом.

Кнопка «Продлить» открывает окно подтверждения оплаты. В нём показываются пакет,
тариф, срок, сумма и способ оплаты.

Нода назначается автоматически; пользователю не показывается выбор страны.

Для ручной оплаты API создаёт платёж `manual_bank` с отметкой
...
Для ручных платежей карточка покупки отправляется админу при создании платежа,
а чек — отдельным уведомлением после загрузки.

Для Platega кабинет создаёт платёж `provider=platega`, сохраняет ссылку
`redirect` и открывает страницу оплаты. Ручной апрув в админке отключён:
подписка создаётся или продлевается только после callback `CONFIRMED` от
Platega. Уведомления админу и клиенту отправляются после подтверждённой оплаты,
а не при создании платежа.
