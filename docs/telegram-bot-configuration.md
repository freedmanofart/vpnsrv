...
Бот показывает страну в сценариях покупки, пробного доступа, промокода и
перевыпуска только тогда, когда логическая нода имеет VLESS-конфигурацию с:

- `api_address`, начинающимся с `http://` или `https://` и указывающим на
  3x-ui master вместе с его web base path;
- числовым `inbound_tag`, равным ID inbound в 3x-ui master.

При выборе новой child-ноды бот вызывает
`POST /subscriptions/{subscription_id}/rotate`. Control plane сначала создаёт
нового клиента `vpn-<id>` в целевом inbound через master и только после успеха
удаляет старого клиента. Поэтому ошибка связи с новой нодой не должна лишать
пользователя действующего ключа.

Для inbound с транспортом XHTTP конфигурация логической ноды дополнительно
содержит `type=xhttp`, `path`, `mode` и `xhttp_host`. При формировании VLESS URI
`xhttp_host` преобразуется в стандартный query-параметр `host`, не подменяя
публичный адрес VPN-ноды.
