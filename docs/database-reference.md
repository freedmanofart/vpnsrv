...
Индекс `user_id + kind + code` используется для подсчёта применений. Допустимое
количество задаёт соответствующая запись `promo_codes`; для старых env-кодов
API сохраняет ограничение в одно применение на пользователя.

### `promo_codes`

Промокоды, привязанные к конкретным тарифам из `plans`.

| Поле | Назначение |
| --- | --- |
| `id` | ID промокода |
| `code` | код без учёта регистра, уникальный |
| `plan_id` | тариф, который выдаётся новому пользователю |
| `max_redemptions` | общий лимит применений; `NULL` означает без ограничений |
| `max_redemptions_per_user` | лимит для одного пользователя; `NULL` означает без ограничений |
| `is_active` | доступен ли код |
| `starts_at`, `expires_at` | необязательное окно действия |
| `created_at`, `updated_at` | даты |

Миграция `c8e4a1f093d2` создаёт `LIGHT1DAY`, связанный с `plans.code = lite_1d`.
Оба лимита равны `NULL`: число пользователей и повторных применений одним
пользователем не ограничено. Каждая выдача отдельно записывается в
`access_grants`.

В VPN Admin есть раздел `Промокоды`. В нём можно создать код, выбрать тариф,
задать общий лимит, лимит на пользователя, период действия и активность, а
также удалить код. Пустой лимит сохраняется как `NULL` и означает отсутствие
ограничения. Удаление записи из `promo_codes` запрещает новые активации, но не
удаляет записи `access_grants` и не отзывает уже выданные подписки.

Административные API:

- `POST /admin/promo-codes` — создать промокод;
- `DELETE /admin/promo-codes/{promo_code_id}` — удалить промокод;
- `GET /admin/overview` — получить список промокодов и число применений для
  таблицы админки.
...
Полный код в БД и в списке админки не хранится. VPN Admin показывает маску по
`code_prefix`; исходное восьмизначное значение возвращается только в ответе на
выпуск/перевыпуск. Новый код завершает срок всех неиспользованных активных кодов
того же пользователя.

Административные API:

- `POST /admin/provider-codes` — выпустить по email или Telegram ID;
- `POST /admin/provider-codes/{activation_id}/refresh` — отменить прежний и
  выпустить новый;
- `GET /admin/overview` → `provider_codes` — история с маскированными кодами.
...
`client_devices.expires_at` следует сроку активной подписки. Истечение записи в
`activation_codes` после успешной активации не отзывает созданное устройство.
...
- Добавлена операционная база:
  ...
  - `access_grants`;
  - `admin_settings`.
- Добавлены промокоды из БД:
  - `promo_codes` связывает код с конкретным `plans.id`;
  - `max_redemptions = NULL` задаёт неограниченное общее число активаций;
  - `max_redemptions_per_user = NULL` разрешает повторные активации тем же
    пользователем;
  - старый `PROMO_CODES` остаётся fallback для ранее настроенных кодов.
